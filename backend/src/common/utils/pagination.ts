...
export class PaginatedResultDto<T> {
  ...
  @ApiProperty({ description: 'The cursor ID to fetch the next sequential page (if any)' })
  nextCursor?: number | string | null;

  @ApiProperty({ description: 'Boolean identifying whether there are more pages available' })
  hasNextPage: boolean;
}
...
export function buildPaginatedResponse<T>(
  ...
  const nextCursor = hasNextPage && edges.length > 0 
    ? (edges[edges.length - 1][cursorField] as any)
    : null;
