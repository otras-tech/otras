import { Controller, Get, Post, Body, Param, Query, UseGuards, UsePipes, ValidationPipe, ParseIntPipe, Request, ForbiddenException } from '@nestjs/common';
...
export class MockTestController {
  ...
  async startAttempt(@Body() dto: StartMockAttemptDto, @Request() req) {
  ...
  @UsePipes(new ValidationPipe({ whitelist: true }))
  async submitAttempt(@Body() dto: SubmitMockAttemptDto, @Request() req) {
    if (req.user.otrId !== dto.otrId) {
  ...
  @UsePipes(new ValidationPipe({ whitelist: true }))
  async submitExamAttempt(@Body() dto: SubmitExamAttemptDto, @Request() req) {
    if (req.user.otrId !== dto.otrId) {
        throw new ForbiddenException('Cannot submit for another user');
    }
  ...
  async getRecentAttempt(@Param('otrId') otrId: string, @Request() req) {
