...
export class ArthaProcessor extends WorkerHost {
  ...
  async process(job: Job<ArthaJobData>): Promise<{ status: string }> {
    const { type, userId, assessmentId, tier, data } = job.data;
    this.logger.log(
      `Processing artha job ${job.id} of type ${type} for user ${userId}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to process artha job ${job.id}`,
  ...
  private async handleTierAnalysis(
    userId: string,
    assessmentId: string,
    tier: number,
  ...
  private async calculateReadiness(
    ...
    if (response.ok) {
      const result = (await response.json()) as { readinessIndex: number };
