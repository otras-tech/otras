...
export class ArthaService {
  ...
  async recordQuestionAttempt(data: {
    ...
    if (assessment) {
      ...
      console.log(
        `Progress = (${attemptedCount} / ${totalQuestionsInTier}) * 100 = ${progress}%`,
      );
  ...
  async processTier1(data: ArthaProgressDto, assessmentId?: string) {
    ...
    if (existingProfile) {
      console.log(
        `ARTHA: Clearing old feedback for user ${data.userId} to ensure fresh analysis`,
      );
    ...
    console.log(`ARTHA Progress Engine: Calculation Result`);
    console.log(
      `Progress = (${attemptedCount} / ${total_questions}) * 100 = ${progress}%`,
    );
    ...
    console.log(
      'ARTHA: Sending Tier 1 data to AI service with language:',
      inputData.language,
    );
    console.log(
      'ARTHA: AI Payload Details:',
      JSON.stringify(inputData, null, 2),
    );
    const feedback = await this.generateAndSaveAiFeedback(
      profile,
      tier,
      inputData,
    );
    ...
    // Only return new feedback; don't fallback to old ones from status.feedback
  ...
  async processTier2(
    ...
    const totalScore = (Object.values(subjectScores) as number[]).reduce(
    ...
    await this.repository.upsertRecentReport(userId, {
      ...
      percentile,
      readinessIndex,
      accuracy: percentile, // Accuracy for Tier 2 is basically the score %
      subjectBreakdown: subjectScores,
    });

    console.log(
      'ARTHA: Sending Tier 2 data to AI service with language:',
      language,
    );
  ...
  async processTier3(
    ...
    if (assessmentId) {
      ...
      try {
        ...
        console.log('ARTHA: Updating Profile Percentile in DB...');
        await this.repository.updateProfilePercentile(
          userId,
          inputData.percentile,
        );
      } catch (err) {
        this.logger.error(
    ...
    console.log(
      'ARTHA: Sending Tier 3 data to AI service with language:',
      language,
    );
  ...
  private async calculateReadiness(
    ...
    console.log(
      `ARTHA Readiness Engine: Raw scores extracted → ${JSON.stringify(currentData)}`,
    );
    ...
    console.log(
      `ARTHA Readiness Engine: Feature vector → [${featureVector.map((v) => Number(v.toFixed(2))).join(',')}]`,
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `ARTHA Readiness Engine: Updating career readiness index in DB`,
        );
        await this.repository.updateProfileReadiness(userId, readinessIndex);

        console.log(
          `ARTHA Readiness Engine: Returning readiness index to frontend.`,
        );
        return readinessIndex;
      } else {
        console.error(
          `ARTHA Readiness Engine ERROR: Model prediction failed status ${response.status}`,
        );
      ...
      console.error(
        `ARTHA Readiness Engine ERROR [Fallback Activated]: ${error.message}`,
      );
      ...
      console.log(
        `ARTHA Readiness Engine: Using fallback readiness index: ${finalReadiness}`,
      );
  ...
  private async generateAndSaveAiFeedback(
    ...
    try {
      console.log('ARTHA: Sending payload to AI Service:', {
        tier,
