import { Injectable, UnauthorizedException, ForbiddenException, Logger, HttpException, HttpStatus } from '@nestjs/common';
...
export interface AuthUser {
    ...
    role: string;
    isDeleted: boolean;
    [key: string]: any; // Allow for other User fields without explicit listing
}
...
export class AuthService {
    ...
    async validateUser(loginId: string, pass: string): Promise<AuthUser | null> {
        ...
        if (targetUser && !targetUser.isDeleted && await bcrypt.compare(pass, targetUser.password)) {
    ...
    async register(data: RegisterDto) {
        ...
        return this.login(user as unknown as AuthUser);
    }
    ...
    async refreshTokens(userId: number, rt: string, jti: string) {
        ...
        if (!lockValue) {
            ...
            throw new HttpException('Too Many Requests - Rotation in progress', HttpStatus.TOO_MANY_REQUESTS);
        }
        ...
        try {
            ...
            if (!rtMatches) {
                ...
                throw new ForbiddenException('Access Denied - Security Breach Detected');
            }
     ...
     */
    private async getTokens(userId: number, email: string, role: string, tx?: Prisma.TransactionClient) {
