...
let GlobalExceptionFilter = GlobalExceptionFilter_1 = class GlobalExceptionFilter {
    logger = new common_1.Logger(GlobalExceptionFilter_1.name);
        ...
        if (status >= 500) {
            this.logger.error(`[${requestId}] ${request.method} ${request.url} [${status}]`, (exception instanceof Error ? exception.stack : undefined) ||
                JSON.stringify(this.redactSensitiveData(exception)), JSON.stringify(logContext));
        }
        else {
