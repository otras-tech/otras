...
let ArthaService = ArthaService_1 = class ArthaService {
    ...
    async getStatus(requesterId, requesterOtrId, requesterRole, userId) {
        ...
        const [profile, hasSubscription, recentReports, selectedExam] = await Promise.all([
    ...
    async startTierAssessment(requesterId, requesterOtrId, requesterRole, userId, tier) {
        if (requesterId.toString() !== userId &&
            requesterOtrId !== userId &&
            requesterRole.toUpperCase() !== 'ADMIN') {
            throw new common_1.ForbiddenException('Cannot start assessment for another user');
        }
        return this.repository.startAssessment(userId, tier);
    }
    async recordQuestionAttempt(requesterId, requesterOtrId, requesterRole, data) {
        ...
        if (requesterId.toString() !== assessment.profile.userId &&
            requesterOtrId !== assessment.profile.userId &&
            requesterRole.toUpperCase() !== 'ADMIN') {
            throw new common_1.ForbiddenException('Cannot record attempt for another user assessment');
        }
    ...
    async processTier1(requesterId, requesterOtrId, requesterRole, data, assessmentId) {
        if (requesterId.toString() !== data.userId.toString() &&
            requesterOtrId !== data.userId.toString() &&
            requesterRole.toUpperCase() !== 'ADMIN') {
            throw new common_1.ForbiddenException('Cannot submit results for another user');
        }
    ...
    async processTier2(requesterId, requesterOtrId, requesterRole, userId, assessmentId, language, attemptedCountOverride, totalQuestionsOverride) {
        if (requesterId.toString() !== userId &&
            requesterOtrId !== userId &&
            requesterRole.toUpperCase() !== 'ADMIN') {
            throw new common_1.ForbiddenException('Access denied');
        ...
        if (!profile)
            throw new common_1.NotFoundException('Artha Profile not found');
        const assessmentData = {
            tier: 2,
    ...
    async processTier3(requesterId, requesterOtrId, requesterRole, userId, assessmentId, language, attemptedCountOverride, totalQuestionsOverride) {
        if (requesterId.toString() !== userId &&
            requesterOtrId !== userId &&
            requesterRole.toUpperCase() !== 'ADMIN') {
            throw new common_1.ForbiddenException('Access denied');
