...
Object.defineProperty(exports, "StudyPlanService", {
    enumerable: true,
    get: function() {
...
const _reschedulerservice = require("./rescheduler.service");
...
let StudyPlanService = class StudyPlanService {
    async generate(dto) {
        try {
            ...
            if (!userExists) {
                throw new _common.NotFoundException(`User with ID ${dto.userId} not found.`);
            }
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new _common.InternalServerErrorException(`AI Service responded with ${response.status}: ${errorText}`);
                }
                ...
                if (e.message.includes('ECONNREFUSED')) {
                    throw new _common.InternalServerErrorException(`AI Service at ${fullUrl} is not reachable. Please ensure the AI service is running.`);
                }
                throw new _common.InternalServerErrorException(e.message || 'AI Service failed');
            }
        } catch (error) {
            this.logger.error('FATAL: StudyPlan Service Error:', error);
            if (error instanceof _common.InternalServerErrorException) throw error;
            throw new _common.BadRequestException(`Backend Error: ${error.message}`);
        }
    }
    ...
    async updateActivityStatus(activityId, userId, status) {
        ...
        try {
            ...
            if (e.code === 'P2025') {
                throw new _common.NotFoundException(`Activity with ID ${activityId} not found.`);
            }
            throw new _common.InternalServerErrorException(`Failed to update activity: ${e.message}`);
        }
        if (status.completed) {
    ...
    constructor(repository, rescheduler){
        ...
        this.logger = new _common.Logger(StudyPlanService.name);
    }
};
StudyPlanService = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", [
        typeof _studyplanrepository.StudyPlanRepository === "undefined" ? Object : _studyplanrepository.StudyPlanRepository,
        typeof _reschedulerservice.ReschedulerService === "undefined" ? Object : _reschedulerservice.ReschedulerService
    ])
], StudyPlanService);
